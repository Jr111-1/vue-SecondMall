            ...
            <div class="detail">
                ...
                <div class="detail-right">
                    ...
                    <div class="rt-item rt-quantity">
                      ...
                      <div class="rt-tag" style="width:70px;margin-left:20px">销量:</div>
                      <div class="rt-tag">{{data.sellnum}}</div>
                      <div class="rt-tag" style="width:70px;margin-left:20px">好评率:</div>
                      <div class="rt-tag">{{this.goodRating}}</div>
                    </div>
                    <div class="rt-item rt-note">
                        <div class="rt-tag">地址</div>
                  ...
                  <div class="rt-item rt-bottom">
                    <el-button type="primary" @click="dialogVisible2 = true">加入购物车</el-button>
                  </div>
                    <el-dialog
                        title="提示"
                  ...
                  <el-dialog
                    ...
                    width="30%"
                    :before-close="handleClose">
                    <span>总价为￥{{totalPrice(data.price*order.num, 1)}}，确定加入购物车吗？</span>
                    <span slot="footer" class="dialog-footer">
                            <el-button @click="dialogVisible = false">取 消</el-button>
                            <el-button type="primary" @click="getinCar()">确 定</el-button>
                        </span>
                  </el-dialog>
                </div>
            </div>
...
export default {
  ...
  mounted () {
    ...
    this.car.commodityId = this.$store.state.commodity.id
    this.car.buyerId = this.$store.state.user.id
    this.initData()
  },
  components: {
    Nav
        ...
        if (resp.status === 200) {
          ...
          if (res.code === 200) {
            ...
            for(let i = 0; i < this.commentList.length; i++ ) {
              this.goodRating += this.commentList[i].rating
            }
            this.goodRating = this.goodRating/this.commentList.length
          }
        }
      })
      this.$axios.get('/apis/commodities/' + this.car.commodityId + '/comments').then(resp => {
        if (resp.status === 200) {
          var res = resp.data
          if (res.code === 200) {
            this.commentList = res.data
