...
<!--              <el-button size="small" type="primary" icon="el-icon-search" @click="searchBooksByKeyWord()">搜索</el-button>-->
<!--            </el-col>-->
<!--          </el-row>-->
      ...
      <div id="content" style="overflow:auto">
        ...
        <div class="com-list"  infinite-scroll-disabled="disabled">
          <div class="com-wrap" v-for="item in commonList" :key="item.id" @click="getCommodityInfo(item)" >
            <div class="com-item-tag"></div>
            <el-image el-image :src="item.photoUrl" fit="fit" class="com-item-img"></el-image>
            <div class="com-item-title">{{item.name}}</div>
            <div class="com-item-price">￥{{item.price}}</div>
          </div>
...
<!--              <el-image :src="item.photoUrl" fit="fit" @click="getCommodityInfo(item)" class="common-item-img"></el-image>-->
<!--            </div>-->
<!--          </div>-->
<!--          <p v-if="loading">加载中...</p>-->
<!--          <p v-if="noMore">没有更多了</p>-->
  ...
  methods: {
    initData () {
      this.$axios.get('/apis/commodities/hot').then((resp) => {
        ...
        this.commonList = resp.data.data.commonList
    ...
    searchByKeyWord () {
      // var _this = this
      this.$axios.get('/apis/commodities/search', { params: { name: this.searchText } }).then(resp => {
        var respData = resp.data
        if (respData.code === 200) {
          var data = respData.data
          this.getCommodityInfo(data)
